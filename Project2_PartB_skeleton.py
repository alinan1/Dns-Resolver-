...
def parse_rr(data, offset):
    ...
    record = {
        "hostname": name,       # owner name of this RR (left side)
        "ttl": ttl,
        "atype": atype,         # numeric type code (1=A, 2=NS, 28=AAAA, ...)
        "rtype": None,          # human label ("A", "AAAA", "NS", or "TYPExx")
        "ip": None,             # populated for A/AAAA glue only
        "nsname": None          # populated for NS only
    }
    if atype == 1 and rdlength == 4:   # A
        record["rtype"] = "A"
        record["ip"] = socket.inet_ntoa(data[rdata_start:rdata_end])

    elif atype == 28 and rdlength == 16:  # AAAA
        record["rtype"] = "AAAA"
        record["ip"] = socket.inet_ntop(socket.AF_INET6, data[rdata_start:rdata_end])
